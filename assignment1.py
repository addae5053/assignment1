...
L = 12
w = 10
x1 = L     # when x is equal 12 in meters
...
print("")
# w*(L/2 - x) = 0 i.e when shear force is zero
# making x the subject from the formula
# x = L/2
...
print("")
Ma = (abs(M))      #absolute values of the bending moment at the various points
Mam = min(Ma)
print(Mam)
#The bending moment formula is M = w*(-6*x**2 + 6*L*x - L**2)/12
# Equating the expression to the minimum bending moment 
# The equation will now be 0.1704 = -6*x**2 + 6*L*x - L**2
...
print("")
#finding the points for the maximum bending moment
